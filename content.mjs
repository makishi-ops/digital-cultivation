...
export const STEPS = [
    ...
    task(0, 'platform-layers', 'order', '拍照指令的三站',
        '預言程式要用鏡頭拍下城門。這道「拍照」指令會依序經過哪三站？從第 1 站排到第 3 站。',
        [
            ['app', '應用程式：預言程式提出「我要拍照」'],
            ['os', '作業系統：接下要求，安排並指揮相機'],
            ['hardware', '硬體：鏡頭與處理器實際拍下影像']
        ], ['app', 'os', 'hardware'], 3, ['指令從提出要求的那一方出發。', '應用程式不能直接指揮零件，要先交給中間那一站。']),
    ...
    task(1, 'mobile-platform', 'choice', '手中的小鏡子',
        '智慧型手機和早期大型電腦相比，最大的改變是什麼？',
        [
    ...
    task(2, 'touchscreen', 'choice', '會發光的鏡面',
        '觸控螢幕屬於哪一種裝置？',
        [
            ['input', '只是輸入裝置'],
            ['output', '只是輸出裝置'],
    ...
    task(3, 'power-loss', 'choice', '停電之後',
        '天機城突然停電，重新開機後，哪一種資料最可能不見？',
        [
            ['ram', 'RAM 裡還沒存檔的資料'],
            ['ssd', '已經存到 SSD 的檔案'],
            ['cloud', '已經上傳到雲端的檔案'],
    ...
    task(5, 'multicore', 'choice', '七位副手',
        '一台電腦的 CPU 有 8 個核心，下列說法何者正確？',
        [
            ['parallel', '可以同時執行多個工作，但不是每個程式都會快 8 倍'],
            ['eight-times', '所有程式都一定快 8 倍'],
            ['eight-gpu', '代表電腦裡有 8 顆 GPU'],
            ['eight-gb', '代表記憶體有 8 GB']
        ], 'parallel', 3, ['一個工作如果必須一步接一步做，多了幫手也不一定變快。']),
    task(5, 'npu', 'choice', '只煉一種丹的丹房',
        '許多新手機裡有 NPU（神經網路處理器），它主要用來做什麼？',
        [
    ...
    task(6, 'edge-cloud', 'choice', '城門守衛鏡',
        '路口攝影機要立刻判斷有沒有人闖紅燈，而且網路可能不穩。辨識工作較適合放在哪裡？',
        [
    ...
    task(7, 'training-data', 'choice', '沒見過的暴雨',
        '天機鏡的模型只用「晴天」的資料訓練，遇到暴雨時最可能發生什麼？',
        [
            ['error', '判斷容易出錯，因為訓練資料沒有涵蓋這種情況'],
            ['perfect', '一樣完全正確，AI 會自己懂'],
            ['faster', '會算得更快'],
            ['same', '不受任何影響']
        ], 'error', 3, ['模型只能從看過的資料裡學習。']),
    task(7, 'verify-output', 'choice', '查證關',
        ...
        'verify', 3, ['信心語氣不是證據。']),
    task(7, 'human-check', 'multi', '三條新門規',
